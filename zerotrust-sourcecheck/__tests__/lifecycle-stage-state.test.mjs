...
const SESSION = "baseline-stage-session";
const BUILD_ROOT = process.cwd();
const CLONE_PATH = process.platform === "win32"
    ? `${BUILD_ROOT}\\zt-stage-test`: `${BUILD_ROOT}/zt-stage-test`;
...
test("baseline active audits initialize at acquired and bind stage state to immutable audit ID", () => {
...
test("baseline analysis stages advance only through the legal ordered sequence", () => {
...
test("baseline analysis stages fail closed on skipped, reversed, stale, or cross-audit transitions", () => {
    const auditId = activate(SESSION, "metadata_only");
    assert.throws(() => advanceAnalysisStage(SESSION, {
            auditId,
            from: "acquired",
            to: "scanned",
        }),
...
test("baseline source audits cannot advance acquired to prepared before index coverage completes", () => {
    const auditId = activate();
    assert.throws(() => advanceAnalysisStage(SESSION, {
            auditId,
            from: "acquired",
...
test("baseline same-stage retries are idempotent when the caller's expected stage is current", () => {
    const auditId = activate();
    const state = advanceAnalysisStage(SESSION, {
        auditId,
        from: "acquired",
...
test("legacy active audit records lazily acquire compatible baseline stage state", () => {
    const auditId = activate();
...
test("current trusted context remains usable without any stage advancement", () => {
    const auditId = activate();
...
test("handler activation initializes stage state without changing packet execution requirements", () => {
    const sessionId = "baseline-handler-stage";
