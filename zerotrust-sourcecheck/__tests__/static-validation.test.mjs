...
const SESSION = "baseline-validation-session";
const BUILD_ROOT = process.cwd();
const LOCAL_PATH = process.platform === "win32"
    ? "C:\\projects\\baseline-validation": "/srv/baseline-validation";
...
function activate({
    ...
    return activateAudit({
        ...
        expectedReportPath: process.platform === "win32"
            ? `${BUILD_ROOT}\\_reports\\local-validation-20260714010000`: `${BUILD_ROOT}/_reports/local-validation-20260714010000`,
...
function confirmArgs(auditId, context, conclusion = "confirmed") {
    ...
    return {
        ...
        rationale_code: conclusion === "confirmed"
            ? "complete-reachable-chain": "activation-not-established",
        rationale: conclusion === "confirmed"
            ? "The existing complete chain establishes activation and effect reachability.": "The supplied context does not establish concrete activation reachability.",
        checks: {
...
function refuteArgs(auditId, context, conclusion = "not-refuted") {
    ...
    return {
        ...
        rationale: supports
            ? "The activation is unreachable in the legitimate project configuration.": "None of the six required alternatives refutes the existing chain.",
        checks: {
            deadOrUnreachableCode: supports
                ? "supports-refutation": "does-not-refute",
...
test("extension registers assignment-bound assurance validation tools", () => {
    const source = readFileSync(
        new URL("../extension.mjs", import.meta.url),
        "utf8",
    );
    assert.match(source, /name:\s*"zerotrust_prepare_assurance_validation"/u);
    assert.match(source, /name:\s*"zerotrust_record_assurance_validation"/u);
    assert.match(source, /name:\s*"zerotrust_finalize_assurance_validation"/u);
    assert.match(source, /\brecordAssuranceValidationHandler\b/u);
    assert.match(source, /independent confirm\/refute assignments for every active severity/u);
    assert.doesNotMatch(source, /validation_min_severity/u);
});
