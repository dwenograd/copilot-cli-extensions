...
test("baseline evidence references are bounded pointers and reject embedded source text", () => {
    ...
    assert.throws(() => validateEvidenceReference({ ...evidence(), sourceText: "malicious" }),
        ContractValidationError,
    );
    assert.throws(() => validateEvidenceReference(evidence({ endLine: LIMITS.line + 1 })),
        /endLine/,
    );
    assert.throws(() => validateEvidenceReference(evidence({ excerptHash: "not-a-hash" })),
        /excerptHash/,
    );
});

test("baseline behavior signatures normalize semantic tokens and reject prose/location fields", () => {
    ...
    assert.throws(() => normalizeBehaviorSignature({
            action: "execute",
            capability: "process-spawn",
            target: "shell",
            line: 17,
...
test("baseline finding IDs depend on collision-resistant source identity and normalized behavior only", () => {
    const first = finding();
...
test("baseline candidate findings strictly separate severity, confidence, and malicious project-fit", () => {
    ...
    assert.throws(() => validateCandidateFinding(finding({ confidence: "critical" })),
        /confidence/,
    );
    assert.throws(() => validateCandidateFinding(finding({ id: `ztf-${"0".repeat(64)}` })),
        /must be derived/,
    );
    assert.throws(() => validateCandidateFinding({ ...finding(), unexpected: true }),
        /unknown field/,
    );
    assert.throws(() => validateCandidateFinding(finding({
...
test("baseline graph contracts enumerate every node kind and reject unknown edge kinds", () => {
    ...
    assert.throws(() => validateGraphNode(graphNode("node.bad", "command")),
        /must be one of/,
    );
    assert.throws(() => validateGraphEdge(graphEdge({ kind: "related-to" })),
        /must be one of/,
    );
});

test("baseline graph documents enforce unique IDs, audit binding, and valid endpoints", () => {
    ...
    assert.throws(() => validateBehaviorGraphDocument({
            ...document,
...
test("BehaviorGraph is bounded, audit-bound, and idempotent only for identical entries", () => {
    ...
    assert.throws(() => graph.addNode(graphNode("node.trigger", "sink")),
        /conflicting/,
    );
    assert.throws(() => graph.addEdge(graphEdge({ id: "edge.missing", to: "node.missing" })),
        /unknown node/,
    );
...
test("validation decisions are strict and finding ledger enforces legal state transitions", () => {
    ...
    assert.throws(() => ledger.applyValidationDecision(decision()),
        /not in validating state/,
    );
    assert.throws(() => ledger.beginValidation(candidate.id, {
    ...
    assert.throws(() => ledger.beginValidation(candidate.id, { auditId: AUDIT_ID }),
...
test("metadata cache documents are bounded typed records with canonical timestamps", () => {
    ...
    assert.throws(() => validateMetadataCacheDocument({
            ...document,
            entries: [
...
test("plugin output validates every nested contract, bounds output, and requires candidate state", () => {
    ...
    assert.throws(() => validatePluginOutput({
            ...output,
    ...
    assert.throws(() => validatePluginOutput({
            ...output,
            warnings: Array.from(
                { length: LIMITS.pluginWarnings + 1 }, () => "bounded warning",
            ),
        }),
        /at most/,
    );
    assert.throws(() => validatePluginOutput({ ...output, rawSource: "do not accept" }),
        /unknown field/,
    );
});
