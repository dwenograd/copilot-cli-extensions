...
function makeScratch(prefix = ".zt-v5-cache-") {
...
test("cache schema is canonical, integrity-checked, and rejects snippet-bearing fields", () => {
    ...
    assert.throws(
        () => validateCachePayload({ ...payload, verdict: "low" }),
    ...
    assert.throws(
        () => parseCacheEnvelope("x".repeat(CACHE_LIMITS.fileBytes + 1)),
        /exceeds/,
    );
...
test("cache paths are versioned, contained, and collision-resistant", () => {
    ...
    assert.match(first.filePath, /_cache[\\/]schema-1[\\/]tool-[a-f0-9]{64}/);
...
test("active-bound store/load persists only normalized metadata and close preserves disk cache", async () => {
    ...
    assert.ok(loaded.pluginRecords.every((record) =>
        record.facts.every((fact) => /^zpcf-v1-[a-f0-9]{64}$/u.test(fact.id))));
