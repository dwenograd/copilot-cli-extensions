...
const SCRATCH = nodePath.join(HERE, ".baseline-dual-finalization-scratch");
const OWNER = "octocat";
...
function session(label) {
    sequence += 1;
    return `baseline-dual-${label}-${sequence}`;
}
...
function installCouncilLedger(sessionId, {
    ...
    const sourceIdentity = {
        ...
        namespace: local
            ? `local-audit:${auditId}`: `github.com/${OWNER}/${REPO}@${SHA}`,
        path: "src/loader.mjs",
        contentSha256: CONTENT,
        blobSha: BLOB,
    };
...
test("baseline council finalization writes and records the canonical pair from one ledger snapshot", async () => {
    ...
    assert.equal(findings.flow, "trusted-ledger");
...
test("baseline rejects model prose and unsafe operator rationale while preserving report/ledger parity", async () => {
    ...
    assert.match(modelProse.textResultForLlm, /model-authored report prose.*refused/i);
    ...
    assert.throws(() => assertMarkdownFindingsConsistency(markdown, findings),
        /finding rows do not match/i,
    );
    assert.throws(() => assertMarkdownFindingsConsistency(
...
test("caller-recorded council outcome is ignored in favor of the deterministic ledger", async () => {
    ...
    const body = parse(result);
    assert.equal(body.verdict, "high");
    assert.equal(body.recordedOutcome.verdict, "high");
    assert.equal(body.recordedOutcome.severityCounts.high, 1);
    assert.equal(body.recordedOutcome.trusted, true);
});
...
test("artifact size caps reject oversized operator rationale and oversized findings serialization", async () => {
    ...
    assert.throws(() => serializeFindingsArtifact(document),
        /FINDINGS\.json exceeds/i,
    );
    assert.throws(() => serializeFindingsArtifact({
            flow: "trusted-ledger",
