...
//
// Anchor tests for security rationale — fixes for the bypasses
// found in the second triple-review pass. security rationale found:
//
//   Security case: value-taking long-flag space form bypassed clone gate
//     (--git-dir /x, --work-tree /x, --exec-path /x, --namespace x)
//   Security case: env-var prefix bypassed (GIT_DIR=/x git clone)
//   Security case: PowerShell call operator (`& "git.exe" clone`)
//                 and dot-source (`. git.exe clone`) bypassed
//   Security case:     bash background single `&` not in splitSubCommands
//                 separators (cmd & git clone)
//   Security case:     PS quote-fragment concatenation (g"it") bypassed
//   Security case:     first-clone-wins — chained `<good> && <bad clone>`
//                 only validated the first hit
//   Security case:     UTF-16-encoded text scripts (.ps1/.bat saved as
//                 UTF-16LE) had nulls → mis-classified as binary
//   Security case:     packet didn't tell agent about coverageComplete
...
function testCloneDeniedInAuditSource(testName, command) {
    test(testName, () => {
        const sid = "clone-argument-validation-" + Math.random().toString(36).slice(2, 8);
        deactivateAudit(sid);
        activateAudit({
            sessionId: sid,
...
testCloneDeniedInAuditSource(
    "clone-argument-validation: git --git-dir /x clone (space-separated value) is DENIED",
    "git --git-dir /tmp/x clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: git --work-tree /x clone (space-separated value) is DENIED",
    "git --work-tree /tmp/y clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: git --exec-path /x clone (space-separated value) is DENIED",
    "git --exec-path /tmp/z clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: git --namespace foo clone (space-separated value) is DENIED",
    "git --namespace foo clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: git -C /path clone (-C with spaced value) is DENIED",
...
testCloneDeniedInAuditSource(
    "clone-argument-validation: GIT_DIR=/x git clone (env-var prefix) is DENIED",
    "GIT_DIR=/tmp/x git clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: multiple env-var prefixes git clone is DENIED",
    "GIT_DIR=/x GIT_WORK_TREE=/y git clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: env-var prefix + global flag git clone is DENIED",
...
testCloneDeniedInAuditSource(
    "clone-argument-validation: PS call operator `& git.exe clone` is DENIED",
    "& git.exe clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: PS call operator with quoted full-path `& \"C:\\..\\git.exe\" clone` is DENIED",
    '& "C:\\Program Files\\Git\\bin\\git.exe" clone https://github.com/x/y ' + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: PS dot-source `. git.exe clone` is DENIED",
    ". git.exe clone https://github.com/x/y " + CLONE_PATH,
);
...
testCloneDeniedInAuditSource(
    "clone-argument-validation: `sudo git clone` is DENIED",
    "sudo git clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: `nohup git clone` is DENIED",
    "nohup git clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: `env git clone` is DENIED",
    "env git clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: `time git clone` is DENIED",
    "time git clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: `exec git clone` is DENIED",
...
testCloneDeniedInAuditSource(
    "clone-argument-validation: bash `cmd & git clone` (single & background) is DENIED",
    "echo hi & git clone https://github.com/x/y " + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    "clone-argument-validation: bash `cmd & git.exe clone` (single & background) is DENIED",
...
testCloneDeniedInAuditSource(
    'clone-argument-validation: PS quote-fragment `g"it" clone` is DENIED',
    'g"it" clone https://github.com/x/y ' + CLONE_PATH,
);
testCloneDeniedInAuditSource(
    'clone-argument-validation: PS quote-fragment `"git""".exe clone` is DENIED',
    '"git""".exe clone https://github.com/x/y ' + CLONE_PATH,
);

// ---------- first-clone-wins fix (validate ALL hits) ----------

test("clone-argument-validation: chained `<good clone> && <bad clone outside build_root>` is DENIED", () => {
    const sid = "clone-argument-validation-chained-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({
        sessionId: sid,
        buildPath: BUILD_ROOT,
...
test("clone-argument-validation: chained `<good clone> && <good clone same path>` is ALLOWED", () => {
    // Sanity: validating ALL hits doesn't break legitimate chained
    // commands where every clone is fine.
    const sid = "clone-argument-validation-chained-good-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({
        sessionId: sid,
        buildPath: BUILD_ROOT,
...
test("clone-argument-validation: UTF-16LE-encoded .ps1 with BOM is classified as TEXT", () => {
...
test("clone-argument-validation: UTF-16BE-encoded .bat with BOM is classified as TEXT", () => {
...
test("coverage security: UTF-16 text under a binary-looking suffix stays text", () => {
...
test("clone-argument-validation: detectUtf16Bom returns correct encoding label", () => {
...
test("clone-argument-validation: packet Section 4 instructs agent on coverageComplete gate", async () => {
...
test("clone-argument-validation: new code (apiClient + command-coverage-boundaries test file) free of contiguous offensive cmdlets", async () => {
    ...
    const filesToScan = [
        ...
        path.join(here, "command-coverage-boundaries.test.mjs"),
        path.join(here, "clone-argument-validation.test.mjs"),
    ];
...
// ---------- security rationale: sub-agent prompt preamble must forbid file writes + force Set-Location ----------
//
...
test("audit_source packet sub-agent preamble forbids file writes", () => {
...
test("audit_source packet sub-agent preamble requires Set-Location buildRoot", () => {
    const packet = buildInstructionPacket(makeBasePacketArgs("audit_source"));
...
test("full_build packet sub-agent preamble forbids file writes (api-direct branch)", () => {
    const packet = buildInstructionPacket({
