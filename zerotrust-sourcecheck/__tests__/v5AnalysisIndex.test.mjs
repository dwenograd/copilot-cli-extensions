...
test("fact extraction produces bounded normalized facts without source excerpts", () => {
    const manifestSource = JSON.stringify({
        ...
        endpoint: "https://user:pass@api.example.com/v1?q=secret#fragment",
    }, null, 2);
    const codeSource = [
...
test("mandatory API fetch indexes full text and advances acquired to prepared", async () => {
    ...
    const text = "export function run(){ return fetch('https://api.example.com/v1'); }";
    ...
    assert.throws(
        () => validateIndexedEvidenceReference(sessionId, {
            auditId: body.analysisStageState.auditId,
            path: "src/index.mjs",
            startLine: 1,
            endLine: 2,
...
test("empty pinned trees prepare without fabricating file facts", async () => {
    const sessionId = "v5-index-empty-api";
    activateAudit({
        sessionId,
        buildPath: process.cwd(),
        mode: "audit_source",
...
test("local ingestion is exact-root-bound, non-executing, source-text-free, and quantitative", async (t) => {
    const sourceRoot = makeScratch(".zt-v5-local-");
    const buildRoot = makeScratch(".zt-v5-buildroot-");
    ...
    const sessionId = "v5-index-local";
    activateAudit({
        sessionId,
        buildPath: buildRoot,
...
test("build ingestion reads only the exact recorded clone", async () => {
    const buildRoot = makeScratch(".zt-v5-build-");
    ...
    const sessionId = "v5-index-build";
    activateAudit({
        sessionId,
        buildPath: buildRoot,
...
test("on-disk ingestion refuses unbound/API roots and detects post-enumeration mutation", async () => {
    const buildRoot = makeScratch(".zt-v5-refuse-");
    const localRoot = makeScratch(".zt-v5-mutate-");
    writeFileSync(nodePath.join(localRoot, "mutable.js"), "const before = 1;");

    activateAudit({
        sessionId: "v5-index-api-refuse",
        buildPath: buildRoot,
        mode: "audit_source",
    ...
    activateAudit({
        sessionId: "v5-index-build-unbound",
        buildPath: buildRoot,
        mode: "audit_and_safe_build",
    ...
    const sessionId = "v5-index-mutation";
    activateAudit({
        sessionId,
        buildPath: buildRoot,
        mode: "audit_local_source",
