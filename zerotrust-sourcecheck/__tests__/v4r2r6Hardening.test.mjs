...
//
// Anchor tests for v4-r2 ROUND-6 hardening:
//   1/3 CRIT (A) — first-rule-wins on install detection
//                  (`npm install --ignore-scripts && pnpm dlx evil` allowed)
//   1/3 HIGH (A) — SHA-binding only fires after safe_list_tree
//                  (direct safe_fetch_file with arbitrary SHA accepted)
//   1/3 HIGH (C) — synthesis fallback misses `npm i` (single-letter alias)
//                  + synthesized npx (no install verb at all)
//   1/3 HIGH (C) — Contents API returns encoding="none" for >1MB files,
...
// ---- A-R6-1: validate ALL install hits across sub-commands ----

test("v4-r2-r6: `npm install --ignore-scripts && pnpm dlx evilpkg` is DENIED in safe-build", () => {
    const sid = "v4r2r6-chained-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
...
test("v4-r2-r6: `npm install --ignore-scripts ; npx evilpkg` is DENIED", () => {
    const sid = "v4r2r6-chained-npx-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({ sessionId: sid, buildPath: BUILD_ROOT, mode: SAFE_BUILD, expectedClonePath: CLONE_PATH });
    const r = inspectToolCall({
        sessionId: sid,
...
test("v4-r2-r6: `npm install --ignore-scripts && go install ./...` is DENIED", () => {
    const sid = "v4r2r6-chained-go-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({ sessionId: sid, buildPath: BUILD_ROOT, mode: SAFE_BUILD, expectedClonePath: CLONE_PATH });
    const r = inspectToolCall({
        sessionId: sid,
...
test("v4-r2-r6: `pip install --no-deps --no-build-isolation foo && cargo install bar` is DENIED", () => {
    const sid = "v4r2r6-chained-cargo-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({ sessionId: sid, buildPath: BUILD_ROOT, mode: SAFE_BUILD, expectedClonePath: CLONE_PATH });
    const r = inspectToolCall({
        sessionId: sid,
...
test("v4-r2-r6: `npm install --ignore-scripts && yarn install --ignore-scripts` is ALLOWED (both safe)", () => {
    const sid = "v4r2r6-chained-good-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({ sessionId: sid, buildPath: BUILD_ROOT, mode: SAFE_BUILD, expectedClonePath: CLONE_PATH });
    const r = inspectToolCall({
        sessionId: sid,
...
// ---- A-R6-2: SHA-binding gate (also blocks direct fetch without prior safe_list_tree) ----

test("v4-r2-r6: safe_fetch_file refuses when no SHA pinned (no prior safe_list_tree)", async () => {
    const sid = "v4r2r6-nopin-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({
        sessionId: sid,
...
test("v4-r2-r6: safe_fetch_file accepts after recordResolvedSha pin (sanity)", async () => {
    const sid = "v4r2r6-pinned-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({
        sessionId: sid,
        buildPath: BUILD_ROOT,
...
// ---- C-R6-2: synthesized npm i + synthesized npx ----

function denyTest(name, command, mode = "audit_source") {
    test(name, () => {
        const sid = "v4r2r6-syn-" + Math.random().toString(36).slice(2, 8);
        deactivateAudit(sid);
        activateAudit({
            sessionId: sid,
            buildPath: BUILD_ROOT,
...
denyTest("v4-r2-r6: `& ([char]110+[char]112+[char]109) i` (synthesized npm i) is DENIED",
    "& ([char]110+[char]112+[char]109) i somepkg");
denyTest("v4-r2-r6: `& ([char]110+[char]112+[char]120) somepkg` (synthesized npx, no verb) is DENIED",
    "& ([char]110+[char]112+[char]120) somepkg");
denyTest("v4-r2-r6: any command with PS synthesis sigil in audit-only mode is DENIED",
    "& ([char]65+[char]66+[char]67) somerandomcommand");

test("v4-r2-r6: PS synthesis is ALLOWED in build mode (sanity — agent uses safe wrappers)", () => {
    // In build mode the wrappers do their own validation; PS synthesis
    // is only blanket-blocked in audit-only modes.
    const sid = "v4r2r6-syn-build-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({
        sessionId: sid,
        buildPath: BUILD_ROOT,
...
test("v4-r2-r6 sanity: legitimate audit command without synthesis is ALLOWED", () => {
    const sid = "v4r2r6-sanity-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({
        sessionId: sid,
        buildPath: BUILD_ROOT,
...
test("v4-r2-r6 sanity: hardened raw git clone in build mode still ALLOWED", () => {
    const sid = "v4r2r6-allow-" + Math.random().toString(36).slice(2, 8);
    deactivateAudit(sid);
    activateAudit({
        sessionId: sid,
        buildPath: BUILD_ROOT,
