...
//
// Tests for the v3.1 security hardening pass — fixes prompted by the first
...
// dedicated file documents the security properties added in the v3.1 round
...
const SESSION = "v31-test-session-" + Math.random().toString(36).slice(2, 8);
...
function activate(sessionId, mode) {
    ...
    // Round-6 hardening: install/build wrappers require resolvedClonePath
...
// =====================================================================
// Cluster M: evaluateCouncilGate — proceed_on_council_failure must NOT
// also bypass severity (the two override flags are strictly orthogonal).
// =====================================================================

test("M: incomplete + critical + only proceed_on_council_failure → still BLOCKED (severity check still applies)", () => {
    const outcome = { verdict: "critical", criticalCount: 1, highCount: 0, complete: false };
    const r = evaluateCouncilGate(outcome, { overrideOnFailure: true });
    assert.equal(r.passes, false);
    assert.match(r.reason, /verdict critical.*does not pass/i);
});

test("M: incomplete + critical + BOTH override flags → opens (both required)", () => {
    const outcome = { verdict: "critical", criticalCount: 1, highCount: 0, complete: false };
    const r = evaluateCouncilGate(outcome, { override: true, overrideOnFailure: true });
    assert.equal(r.passes, true);
    assert.match(r.reason, /INCOMPLETE.*council_build_override/i);
});

test("M: incomplete + low verdict + only proceed_on_council_failure → opens (severity passes)", () => {
    const outcome = { verdict: "low", criticalCount: 0, highCount: 0, complete: false };
    const r = evaluateCouncilGate(outcome, { overrideOnFailure: true });
    assert.equal(r.passes, true);
    assert.match(r.reason, /INCOMPLETE.*proceed_on_council_failure/i);
});

test("M: complete + critical + only council_build_override → opens (incompleteness passes)", () => {
    const outcome = { verdict: "critical", criticalCount: 1, highCount: 0, complete: true };
    const r = evaluateCouncilGate(outcome, { override: true });
    assert.equal(r.passes, true);
});

test("M: complete + critical + only proceed_on_council_failure → BLOCKED (severity bypass not granted)", () => {
    const outcome = { verdict: "critical", criticalCount: 1, highCount: 0, complete: true };
    const r = evaluateCouncilGate(outcome, { overrideOnFailure: true });
    assert.equal(r.passes, false);
});
...
test("A: safeBuildHandler enforces council gate when active audit is council-build, even if args.mode says non-council", async () => {
...
test("A: safeBuildHandler enforces council gate when active audit is council-build, even if args.mode is omitted", async () => {
    activate(SESSION, "audit_and_safe_build_council");
    const r = await safeBuildHandler(
...
test("A: safeBuildHandler does NOT enforce gate when active audit is non-council-build", async () => {
    ...
    // Will fail at execFileSync (no real clone), not at the gate
    assert.equal(r.resultType, "failure");
    assert.doesNotMatch(r.textResultForLlm, /council-build gate/);
});

// =====================================================================
// Cluster D + I: cleanupAuditHandler containment hardening.
// =====================================================================
...
test("R2/B: install rejects bare --no-binary (split-form bypass)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--no-binary", ":all:"]),
...
test("R2/B: install rejects bare --only-binary", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--only-binary", ":none:"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R2/B: install rejects bare --prefix", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--prefix", "."]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R2/B: install rejects bare --use-feature", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--use-feature", "no-build-isolation-fallback"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R2/B: install rejects absolute Windows path in extra_args", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["C:\\Users\\testuser\\evil"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R2/B: install rejects path-traversal in extra_args", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["../escape"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R2/B: build rejects bare --manifest-path (split-form)", () => {
    assert.throws(
        () => buildInternals.validateExtraArgs(["--manifest-path", "/etc/evil/Cargo.toml"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R2/B: build rejects absolute path positional arg", () => {
    assert.throws(
        () => buildInternals.validateExtraArgs(["C:\\Users\\testuser\\evil\\app.sln"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R2/6: parseGithubUrl supports slash-containing release tags (e.g. v1/2)", () => {
    const r = parseGithubUrl("https://github.com/octocat/Hello-World/releases/tag/v1/2");
    assert.equal(r.ok, true);
    assert.equal(r.parsed.kind, "release");
    assert.equal(r.parsed.ref, "v1/2");
});

// =====================================================================
// ROUND 3 anchors below.
// =====================================================================
...
test("R3/2-UNC: install rejects UNC absolute path in extra_args", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["\\\\server\\share\\evil.whl"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R3/2-UNC: install rejects bare-leading-backslash path", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["\\foo\\bar"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R3/2-UNC: build rejects UNC absolute path in extra_args", () => {
    assert.throws(
        () => buildInternals.validateExtraArgs(["\\\\evil-server\\share\\app.csproj"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R3/4-flag-value: install rejects --target=C:\\evil (absolute path in flag value)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--target=C:\\evil"]),
...
test("R3/4-flag-value: install rejects --output=\\\\server\\share (UNC in flag value)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--output=\\\\server\\share"]),
...
test("R3/4-flag-value: install rejects --cache-dir=../escape (traversal in flag value)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--cache-dir=../escape"]),
...
test("R3/4-flag-value: build rejects --output=C:\\bad (absolute path in flag value)", () => {
    assert.throws(
        () => buildInternals.validateExtraArgs(["--output=C:\\bad"]),
        /negate hardcoded safety|positional|absolute path/i,
    );
});

// =====================================================================
// ROUND 4 anchors below.
// =====================================================================

test("R4/registry: install rejects --index-url=https://attacker/ (pip)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--index-url=https://attacker.example/"]),
...
test("R4/registry: install rejects bare --extra-index-url (split-form)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--extra-index-url", "https://attacker/"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R4/registry: install rejects --trusted-host=attacker", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--trusted-host=attacker.example"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R4/registry: install rejects --registry=https://attacker/", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--registry=https://attacker/"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R4/registry: build rejects --source=https://attacker/ (dotnet/cargo)", () => {
    assert.throws(
        () => buildInternals.validateExtraArgs(["--source=https://attacker/"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R4/lockfile: install rejects --no-package-lock", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--no-package-lock"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
// =====================================================================
// ROUND 5 anchors below.
// =====================================================================

test("R5/positional-url: install rejects positional URL arg", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["https://attacker.example/evil.whl"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R5/positional-url: install rejects ftp:// positional", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["ftp://attacker/x.tgz"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R5/positional-url: install rejects file:// positional", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["file:///etc/evil"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R5/version-pin: install rejects pip-style version-pin (pkg==1.2.3)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["malicious-pkg==1.2.3"]),
        /negate hardcoded safety|positional|positional/,
    );
});
...
test("R5/url-in-flag-value: install rejects --cache-dir=https://attacker/", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--cache-dir=https://attacker.example/"]),
...
test("R5/url-in-flag-value: build rejects --output=https://attacker/", () => {
    assert.throws(
        () => buildInternals.validateExtraArgs(["--output=https://attacker.example/"]),
        /remote URL|absolute path|negate hardcoded safety|positional/i,
    );
});
...
// =====================================================================
// ROUND 6 anchors below.
// =====================================================================

test("R6/scoped-pkg: install rejects npm scoped package version-pin (@scope/pkg@1.2.3)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["@scope/malicious-pkg@1.2.3"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R6/scoped-pkg: install rejects bare npm scoped package", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["@scope/malicious-pkg"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R6/scoped-pkg: install rejects @scope/pkg@latest", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["@scope/malicious-pkg@latest"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
// =====================================================================
// ROUND 7 anchors below.
// =====================================================================

test("R7/ignore-scripts-true: install rejects --no-ignore-scripts=true", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--no-ignore-scripts=true"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R7/ignore-scripts-true: install rejects --no-ignore-scripts=1", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--no-ignore-scripts=1"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R7/ignore-scripts-true: install rejects bare --ignore-scripts (already hardcoded)", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["--ignore-scripts=anything"]),
        /negate hardcoded safety|positional|positional/i,
    );
});
...
test("R8/F1-ref-binding: safe_clone refuses different ref of same repo", async () => {
    ...
    const r = await safeCloneHandler(
        { url: "https://github.com/octocat/Hello-World", ref: "v2.0" },
        { sessionId: SESSION },
    );
    assert.equal(r.resultType, "failure");
...
test("R8/F2-bare-pkg: install rejects bare positional package name", () => {
    assert.throws(
        () => installInternals.validateExtraArgs(["malicious-pkg"]),
        /positional/i,
    );
});
...
test("R9/F1-omitted-ref: safe_clone with bare URL uses pinned audit ref (no fall-through to HEAD)", async () => {
    ...
    // path that ls-remote will reject — proves we used ctx.ref="v1.0", not HEAD.
    ...
    assert.equal(r.resultType, "failure");
    // Expect the failure to be about ls-remote not finding v1.0 (we're not actually
