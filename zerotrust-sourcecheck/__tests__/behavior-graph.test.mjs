...
test("traces all prioritized named chains and preserves explicit cross-file topology", () => {
    ...
    assert.ok(traced.chains.every((chain) => /^ztc-[a-f0-9]{64}$/u.test(chain.id)));
});
...
test("trace stage exposes blockers before scan and advances scanned to traced idempotently", async () => {
    const sessionId = "baseline-behavior-stage";
    ...
    const auditId = activateAudit({
        ...
        expectedReportPath: process.platform === "win32"
            ? `${process.cwd()}\\_reports\\local-trace-20260713235500`: `${process.cwd()}/_reports/local-trace-20260713235500`,
...
test("scanned cannot advance to traced through the generic stage API without trace gates", () => {
    const sessionId = "baseline-behavior-stage-gate";
    const auditId = activateAudit({
        sessionId,
        buildPath: process.cwd(),
        mode: "audit_local_source_council",
        ...
        expectedReportPath: process.platform === "win32"
            ? `${process.cwd()}\\_reports\\local-trace-gate-20260713235500`: `${process.cwd()}/_reports/local-trace-gate-20260713235500`,
    ...
    assert.throws(() => advanceAnalysisStage(sessionId, {
            auditId,
            from: "scanned",
            to: "traced",
        }),
...
test("trace wrapper rejects audit identity mismatch", async () => {
    const sessionId = "baseline-behavior-identity";
    activateAudit({
        sessionId,
        buildPath: process.cwd(),
        mode: "audit_local_source_council",
        ...
        expectedReportPath: process.platform === "win32"
            ? `${process.cwd()}\\_reports\\local-identity-20260713235500`: `${process.cwd()}/_reports/local-identity-20260713235500`,
        councilRoleManifest: [{ id: "trace-role", category: "A", mandatory: true }],
    });
