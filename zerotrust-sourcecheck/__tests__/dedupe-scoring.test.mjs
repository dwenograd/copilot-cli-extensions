...
test("decision snapshots are deterministic, identity-bound, bounded, and source-text-free", () => {
    ...
    assert.throws(() => buildTrustedDecisionSnapshot({
            ...input,
...
test("packet hands the validated assurance snapshot to deterministic dual rendering", () => {
    const validatePacket = readFileSync(
        new URL("../packet/validate.mjs", import.meta.url),
        "utf8",
    );
    const finalizePacket = readFileSync(
        new URL("../packet/finalize.mjs", import.meta.url),
        "utf8",
    );
    assert.match(validatePacket, /validationFinal\.analysisSnapshot/u);
    assert.match(validatePacket, /wrapper-owned validated\s+assurance state/u);
    assert.match(validatePacket, /REPORT\.md plus the source-text-free\s+FINDINGS\.json/u);
    assert.match(finalizePacket, /deterministically derives the findings verdict, separate assurance result, counts/u);
    assert.match(finalizePacket, /Do not assemble REPORT\.md or FINDINGS\.json/u);
});
