...
import { modeIsBuild, modeIsCouncilBuild, modeUsesApiDirect, modeUsesCouncil } from "../modes.mjs";
...
This mode does NOT clone, audit source contents, or build. Before Section 8,
...
Then go directly to the single Section 8 finalizer. Pass the full returned
...
| \`audit_source\` | **API-direct** (\`zerotrust_safe_list_tree\` + \`zerotrust_safe_fetch_file\`); no clone or wrapper-created source tree. Deterministic audit. |
| \`audit_source_council\` | **API-direct**; no clone or wrapper-created source tree. Deterministic audit + 32-role council. |
| \`verify_release\` | **API-direct** for source context; release artifacts go to \`_quarantine/\`. No build. |
| \`audit_and_safe_build\` | Hardened on-disk clone + shared install/build wrappers. Install lifecycle scripts stay suppressed; build-time repo code may execute. |
| \`audit_and_full_build\` | Same wrapper commands as safe-build; additionally requires \`unsafe\` for admission/warning posture and reserves a future distinction. |
| \`audit_and_safe_build_council\` | Council audit + shared install/build wrappers; \`zerotrust_safe_build\` refuses until a passing council outcome is recorded or override supplied. |
| \`audit_and_full_build_council\` | Same wrappers and recorded-outcome gate as the safe council build; additionally requires \`unsafe\`. |
`;
...
The immutable active-audit ID for this packet is **\`${auditId || "<unavailable: no session identity>"}\`**. Any council outcome must echo this exact value as \`audit_id\`.
...
Record the **full 40-character SHA**. Wrapper-returned canonical artifact
paths use \`zt-v1-<sha256-identity>\`, where the digest binds an unambiguous,
case-normalized tuple of owner, repo, and the full SHA:
- Clone: \`${buildRoot}\\zt-v1-<64-hex-sha256-identity>\`
- Report: \`${buildRoot}\\_reports\\zt-v1-<64-hex-sha256-identity>\`
- Quarantine: \`${buildRoot}\\_quarantine\\zt-v1-<64-hex-sha256-identity>\`
