...
export function normalizeCoverageSnapshot(snapshot = {}, sourceKind = "unknown") {
    ...
    return {
        ...
        aggregateEntryCount: Number.isInteger(snapshot.aggregateEntryCount)
            ...
            : Number.isInteger(snapshot.totalEntryCount) ? snapshot.totalEntryCount : null,
...
function renderRuntimeContext({
    ...
    const identityLines = sourceKind === "local"
        ...
        : sourceKind === "api-direct"
            ...
            : `- Source kind: hardened build clone\n- Pinned commit SHA: \`${sourceCommitSha}\`\n- Clone path: \`${clonePath}\``;
...
export function renderRolePrompt(role, {
    ...
    const sourceNamespace = localSource
        ? `local-audit:${activeAuditId}`
        : `github.com/${String(owner || "").toLowerCase()}/${String(repo || "").toLowerCase()}@${sha}`;
...
- Binaries: if you encounter a binary file (\`.exe\`, \`.dll\`, \`.so\`, \`.dylib\`, \`.pyd\`, \`.wasm\`, etc.) NOT under \`vendor/\` or \`third_party/\`, that is itself a finding — note the path + size, do NOT view.`
        : apiDirect
...
- Do NOT attempt \`view\`/\`grep\`/\`glob\` against any local path; there are no local source files. Do NOT call \`zerotrust_safe_clone\` or any package-manager install. Binary files return metadata only; use size + sha256 + magic-byte preview and do not request full content.`
            : `- The repository is ALREADY CLONED at the wrapper-returned path \`${cp}\`, pinned to commit \`${sha}\`. Use that exact path; do not reconstruct it from owner/repo/SHA and do not use any pre-acquisition path.
...
\`\`\`json
{
  ...
  "source_identity": ${localSource
        ? `{ "kind": "local", "local_path": ${JSON.stringify(lp)} }`
        : `{ "kind": "git", "owner": ${JSON.stringify(owner)}, "repo": ${JSON.stringify(repo)}, "resolved_sha": ${JSON.stringify(sha)} }`},
  "coverage_performed": [
  ...
  "candidates": [
    {
      "finding": {
        ...
        ? ',\n          "blobSha": "<indexed 40-hex API git blob SHA>"'
        : localSource
