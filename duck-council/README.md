...
Critique a topic with **6 role-specialized rubber-duck reviewers and an optional judge synthesis pass.** Sibling to `triple-duck` — use this when you want different angles rather than consensus across the same angle.
...
| Broad, role-separated pre-implementation review | `duck-council` (default) |
| You only have a small change to review | `triple-duck` (council is overkill) |

Worst-case reservation is 14 calls versus triple-duck's 8. In return, the
council uses six distinct role lenses instead of three same-prompt reviews.
...
Tiered for cost/quality balance. Family-diverse: 4 Claude + 2 GPT reviewers, plus a GPT-5.6 Sol judge.
...
| maintainer | `claude-opus-4.7-1m-internal` alias → base `claude-opus-4.7` | `claude-opus-4.6` |
| skeptic | `gpt-5.4` | `gpt-5.4` |
| user | `claude-sonnet-4.6` | `claude-sonnet-4.6` |
| **judge** | `gpt-5.6-sol` | `claude-opus-4.7` |

Why tiered: UX does not need the same model tier as security/stability. The judge adjudicates all surviving critiques and defaults to GPT-5.6 Sol.
...
Surviving role outputs are preserved verbatim in a collapsible `<details>`
appendix; failed roles are represented by explicit failure placeholders.
...
duck-council({
  ...
  skip_judge?: boolean,               // true = raw outputs only
...
- `skip_judge: true`: **6 + 6 = 12 max.** It removes one normal judge call and its one-call retry allowance; a successful run therefore saves one actual call, while the enforced worst-case reservation drops by two.
...
```
duck-council({
  topic: "We're about to merge a rewrite of the auth flow that uses signed cookies instead of JWTs. Should we ship this on Friday?",
...
```
duck-council({
  topic: "Should I add a Redis cache layer for the catalog endpoint?",
  cheap: true
})
```
...
```
duck-council({
  topic: "Critique this README rewrite",
  context: "...",
  skip_judge: true
})
```
...
```
duck-council({
  topic: "Should this endpoint be public?",
...
- Every spawned role and judge gets `context_tier:"long_context"`. Full-quality mode requests elevated (`xhigh`) effort only for supported base models; cheap mode suppresses automatic elevation. An explicit judge effort alias remains pinned because `judge` is compatible with cheap mode.
